...
class Adapter_XL(nn.Module):
    ...
    def forward(self, x, t=None):
        ...
        output_size = (b, 640, (x[0].shape)[2] * 4 , (x[0].shape)[3] * 4)
        for i in range(len(self.channels)):
...
class UNet2DConditionAdapterModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
...
class StableDiffusionXLAdapterPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    r"""
    ...
    def __call__(
            ...
            negative_prompt_sd1_5: Optional[Union[str, List[str]]] = None,
        ...
        prompt_embeds_sd1_5 = self._encode_prompt_sd1_5(
            ...
            negative_prompt=negative_prompt_sd1_5,
...
class StableDiffusionXLAdapterControlnetPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion XL.
...
class StableDiffusionXLAdapterControlnetI2IPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion XL.
...
def load_adapter_lora(pipeline, lora_model_path, alpha, device):
    state_dict = safetensors.torch.load_file(lora_model_path, device=device)
...
def unload_adapter_loras(pipeline):
    for _, module in pipeline.unet_sd1_5.named_modules():
        for key in list(module.__dict__.keys()):
            if "lora" in key:
                delattr(module, key)

    for _, module in pipeline.text_encoder_sd1_5.named_modules():
        for key in list(module.__dict__.keys()):
            if "lora" in key:
                delattr(module, key)